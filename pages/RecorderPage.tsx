...
import { Mic, Square, RefreshCcw, Loader2, Sparkles, History, Activity, Hash, Clock, LogOut } from 'lucide-react';
...
export const RecorderPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-[#050505] selection:bg-yellow-500/30 selection:text-yellow-200 font-sans text-white">
            ...
            {/* Overriding Navbar for this specific protected page to include Logout */}
            <nav className="fixed w-full z-50 top-0 left-0 bg-black/80 backdrop-blur-md border-b border-white/5 h-20 flex items-center justify-between px-6 lg:px-12">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-600 to-yellow-400 animate-pulse-slow shadow-lg shadow-yellow-500/20"></div>
                    <div className="font-display font-bold text-xl tracking-wide">
                        NEON<span className="text-yellow-500">VOICE</span>
                    </div>
                </div>

                <div className="flex items-center gap-6">
                    <span className="text-xs font-mono text-white/40 hidden md:block">
                        Logged in as <span className="text-yellow-500/80">{user?.email}</span>
                    </span>
                    <button
                        onClick={signOut}
                        className="flex items-center gap-2 text-sm text-red-400 hover:text-red-300 transition-colors font-mono uppercase tracking-wider"
                    >
